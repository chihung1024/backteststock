...
本文件定義個股掃描、投資組合回測與全量最佳化共用的底層數據契約。`api/metrics.py` 為績效計算核心，定義版本為 `2026-08-01.2`；公司行為稽核版本為 `2026-08-01.2`。所有正式估值路徑先依 [`UNIFIED_TWD_CONTRACT.md`](UNIFIED_TWD_CONTRACT.md) 轉為 TWD。
...
- `Adj Close`：Yahoo 調整收盤價，是本地貨幣總報酬的唯一價格來源。
...
```text
TWD adjusted close[t] = native Adj Close[t] × FX(native currency → TWD, t)
```

每檔標的以本地市場與 FX 市場日曆的聯集估值；在 FX-only 日保留前一個已觀測到的還原股價，在本地市場-only 日保留前一個已觀測到的 FX，且絕不以未來資料反向填補。跨市場投組再取各 TWD 估值日曆的聯集，僅在每檔都已實際開始有歷史後形成共同日曆。API metadata、掃描列與全量快照都保存 TWD 合約版本、來源幣別與 FX 稽核資訊。
