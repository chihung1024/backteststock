...
- 瀏覽器每 100 檔循序呼叫 `/api/scan`，但 API 不設 100 檔的人工作業上限；Python 行情下載也以 100 檔為內部分批單位。
- Python 行情層優先使用多股票 `download()`，每次回傳後逐檔驗證，最多嘗試 3 輪且每輪只補抓尚未解析的股票。每個成功序列再解析實際報價幣別與對 TWD FX，形成每日 TWD 還原股價；批次 HTTP 200 不再等同整批成功。
- 每批回應逐檔核對。資產的下載、幣別、FX 或估值失敗都會明列為該 ticker 的可重試結果，瀏覽器只將該股票重新排隊；比較基準失敗不會清除已成功資產，而是明示 Beta／Alpha 暫不計算。
