...
Current production `main` checkpoint at this handoff:

`670086fdb7a74a054bdd39e69ec38aa9ad5d6e8d`

Phase 5 clustering/redundancy methodology and implementation are merged and authoritative on `main`; **P5-CLOSE remains open** because the permanent bounded Refinery production smoke in PR #75 has not yet been reconciled/merged and exercised in production.

The next implementation batch after this documentation convergence is:

**C1-A / PR #75 — reconcile the previously validated Refinery production-smoke candidate with current `main`, then obtain fresh exact-head gates.**
...
- `main`: `670086fdb7a74a054bdd39e69ec38aa9ad5d6e8d`.
...
Always re-query current remote truth before any merge/deployment action; the SHAs above are recovery/evidence anchors, not permission to skip freshness checks.
...
### C0-B — Documentation truth recovery — THIS HANDOFF ARTIFACT

This update is a docs-only convergence based on verified `main@670086f...`. It supersedes the stale execution snapshot on production main and absorbs the useful authority corrections prepared in PR #81 without merging #81's older internal branch wholesale.
...
Current remote truth is different:

- current production main: `670086f...`;
- #75 remains based on merge base `2c9ed83...`;
- compare against current main: **diverged / ahead 9 / behind 2**;
...
- current production main itself has since received a genuine Vercel production SUCCESS.

Therefore the current blocker is **not accurately described as “quota only.”** The candidate is stale relative to current main and must be reconciled, then all applicable exact-head gates must be fresh.

C1-A contract:

1. start from latest verified `main`;
2. reconcile only the four-file #75 smoke/deployment scope;
3. preserve #83/#84 scanner changes and all current-main behavior;
...
| C0-B | Documentation truth recovery | **CURRENT DOC CONVERGENCE** |
...
Decision: `AI_PROJECT_PLAYBOOK.md` remains engineering governance authority. Operational progress is not a Reopen Condition.

Status: LOCKED.

Reopen only under a documented V3 Reopen Condition.

### D-02 — Contract authority is separate from operational closeout

Decision: merged Phase 5 clustering/redundancy contract `.2` and API response schema `.3` are current main authorities even while P5-CLOSE production-smoke acceptance remains pending.

Status: LOCKED.

### D-03 — No branch-protection / Vercel bypass

Decision: external quota/control-plane failures are classified separately but do not justify weakening required checks, force merge, or no-op commits.

Evidence: #84's initially stuck required `validate` check was resolved by rerunning the same exact-head workflow, not by bypass.

Status: LOCKED.

### D-04 — #79 is a comparison-context authority defect

Decision: fix #79 by carrying one authoritative common comparison context/effective bounded benchmark source across the Portfolio service/API boundary. Do not patch only `_benchmark_payload()` with an ad-hoc date slice.

Status: IMPLEMENTATION-READY / issue #79 remains execution authority.

### D-05 — #80 fixed scope must remain convergent

Decision: #83 and #84 close two proven root/presentation defects. Do not reopen cache/back-end orchestration merely because Issue #80 is still open; first reconcile the original acceptance matrix and isolate only remaining reproducible gaps.

Status: LOCKED FOR C4 REVIEW.

### D-06 — Phase 6 planning is saturated

Decision: Issue #77 is the authoritative frozen Phase 6 V1 plan. No additional Phase 6 features/formulas/ranking/sizing work before unlock.
...
Symptom: later chunks/retries could repeatedly receive the same partial HTTP-200 scan result and appear to stall around 300/500.

Root cause: Cloudflare admitted `/api/scan` responses to edge cache without proving all requested symbols resolved; retryable symbol failures inside HTTP 200 could therefore be cached.

Fix: PR #83 caches scan responses only when valid `X-Scan-Requested` and `X-Scan-Resolved` prove complete resolution; missing/invalid/mismatched evidence fails closed. Existing backtest cache behavior remains separate.

Verification: merged to `main@b937b9e...`, main CI/Portfolio/Cloudflare production verification passed.

### RC-80-B — Settled count mislabeled as successful/completed count — RESOLVED

Symptom: UI could show `已完成 400 / 500` while only 300 were successful because terminal failures also live in `job.results`.

Root cause: presentation semantics treated `job.results.length` as success/completion rather than settled count.

Fix: PR #84 adds pure settlement presentation semantics and reports settled/success/failed/unfinished truthfully without changing scanner orchestration.

Verification: merged to `main@670086f...`; Full CI, Portfolio CI, Cloudflare production deploy/smokes and Vercel all PASS.

### RC-78 — Scanner/optimizer migration authority mismatch — OPEN / R1

Symptom: selected scan tickers may fail to auto-populate optimizer for restored legacy `scan-job-v3` data.
...
- restore remote-truth alignment from latest verified main;
...
- supersede the useful content of #81 onto a current-main-based documentation branch rather than merging the old branch wholesale.
...
Status: OPEN / reconciliation required. The old candidate is two current-main commits behind and its old Vercel check remains failed; revalidate on a current-main candidate.

### #79 — P0 financial/backtest correctness

Status: OPEN / implementation-ready. Must precede #80 closeout/#78/Phase 6 after Deployment Economy.

### #80 — Scanner reliability umbrella

Status: OPEN / acceptance reconciliation pending.

Resolved children/evidence:
...
- compare original Issue #80 mandatory test/behavior matrix against landed #83/#84 coverage;
- verify whether any true whole-job interruption remains after root cache fix;
- residual presentation candidate: retry-requeued chunk range can temporarily be inferred from settled count and show a misleading range (for example first 401–500 request displayed as 301–400). Treat as a separate narrow NEXT item only if still reproducible/valuable.

### #78 — Scanner → optimizer manual handoff

Status: OPEN / root cause identified / R1 implementation-ready after #80 acceptance closeout.

### #76 — Deployment Economy

Status: internal-stage PASS only. Candidate/main behavior remains unproven and must be demonstrated after P5-CLOSE.
...
- variant-level full Phase 5 bootstrap within marginal experiments unless later performance/methodology evidence justifies it;
...
- no-op/empty commits to consume/retry deployment quota;
- increasing scanner chunk size merely to hide a reliability symptom;
- hand-merging generated bundles;
- forced dependency remediation (`--force`, blanket audit fix) without evidence;
...
| --- | --- |
| Live docs drift from remote truth | Remote state has precedence; C0-B restores the snapshot; re-query before acting |
| #75 carries stale base / old Vercel evidence | Reconcile from current main and rerun all applicable exact-head gates |
...
**Safety / data integrity / production stability > current feature > optimization.**
...
- Publish this current-main documentation convergence through normal PR/review/CI.
...
2. create a recovery/current-main reconciliation path for #75 without expanding its four-file scope;
3. preserve current `package.json` additions from #83/#84 while adding only #75 smoke/test wiring;
4. verify the reconciled diff contains no scanner/runtime/methodology changes outside the smoke/deploy gate;
5. run exact-head Full CI + Portfolio CI + genuine required Vercel + applicable recovery gate;
6. perform exact-head Independent Review; findings only BLOCKER/FOLLOW-UP/BACKLOG/REJECT;
7. BLOCKER=0 + all gates green → expected-head squash merge #75;
8. require new-main CI / Portfolio / Vercel / backup where applicable and Cloudflare deployment with Russell + Portfolio + **Refinery Phase 5** smoke actually PASS;
9. only then record Phase 5 **CLOSED / PASS** and proceed to #76.
